...
#### 3.2.3 종목 검색 기능
- **FR-009-1**: 사용자는 종목 마스터에서 종목을 검색할 수 있다.
  - 검색 대상: 티커 심볼, 회사명
  - 실시간 자동완성 (입력 시 즉시 결과 표시)
  - 대소문자 구분 없이 검색
- **FR-009-2**: 검색 결과는 최대 20개까지 표시한다.
- **FR-009-3**: 검색 결과에는 다음 정보가 포함된다.
  - 티커 심볼
  - 회사명
  - 섹터 (Industry/ETF 분류)
  - 거래소 정보
- **FR-009-4**: 검색 결과에서 종목 선택 시 관심 종목으로 즉시 추가된다.
- **FR-009-5**: 이미 관심 종목에 있는 종목은 검색 결과에서 구분 표시된다.
...
- **FR-042**: 종목 추가 폼 (티커 심볼 입력)
  - **FR-042-1**: 검색창에 입력 시 실시간 자동완성 드롭다운 표시
  - **FR-042-2**: 드롭다운에서 종목 선택 또는 Enter로 추가
  - **FR-042-3**: 검색어가 2글자 이상일 때 자동완성 활성화
- **FR-043**: 종목 삭제 버튼
